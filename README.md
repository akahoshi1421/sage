...
| `playground/`                             | 動かして確かめるための環境 (Rails アプリ、Go モジュールなど)。`/sage-create` が必要に応じて構築し、`/sage-mark` はここで実行してよい |
...
| `languageServers` | 言語 ID → `{ command, args, cwd }`。宣言が無い言語は既定の候補 (下記) のうち PATH にあるものを使う。`cwd` (例: `playground`) を書くとそこで起動し、ワークスペースもそこになる |
